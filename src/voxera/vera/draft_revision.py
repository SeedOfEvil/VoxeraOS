...
def extract_content_refinement(
    ...
    lowered: str,
    *,
    extract_quoted_content: Any,
    extract_content_after_markers: Any,
...
def refined_content_from_active_preview(
    *,
    ...
    explicit = extract_content_refinement(
        text,
        lowered,
        extract_quoted_content=extract_quoted_content,
        extract_content_after_markers=extract_content_after_markers,
    )
    if explicit:
        return explicit
...
def draft_revision_from_active_preview(
    message: str,
    active_preview: dict[str, Any] | None,
    *,
    current_goal: str,
    normalize_open_goal: Any,
    url_matcher: Any,
    domain_matcher: Any,
    extract_quoted_content: Any,
    extract_content_after_markers: Any,
    ...
    url_match = url_matcher.search(text) or domain_matcher.search(text)
    if (
        url_match
        and current_goal.startswith("open ")
        and re.search(r"\b(actually|instead|change|switch)\b", lowered)
    ):
        normalized_open = normalize_open_goal(f"open {url_match.group(0)}")
        if normalized_open:
    ...
    if content_refinement_intent:
        ...
        refined_content = extract_content_refinement(
            text,
            lowered,
            extract_quoted_content=extract_quoted_content,
            extract_content_after_markers=extract_content_after_markers,
