...
def run(source_path: str, destination_path: str, overwrite: bool = False) -> RunResult:
    try:
        ...
        return RunResult(
            ...
            data={
                SKILL_RESULT_KEY: build_skill_result(
                    ...
                    operator_note="Provide an existing source file under notes scope.",
        ...
        return RunResult(
            ...
            data={
                SKILL_RESULT_KEY: build_skill_result(
                    summary="Rejected file copy outside allowlist",
                    machine_payload={
                        "source_path": source_path,
                        "destination_path": destination_path,
    ...
    if not source.is_file():
        return RunResult(
            ...
            data={
                SKILL_RESULT_KEY: build_skill_result(
                    summary="Rejected non-file source",
                    machine_payload={"source_path": str(source)},
                    operator_note="Copy source path must be a file.",
                    next_action_hint="provide_source_file",
                    retryable=False,
                    blocked=False,
                    approval_status="none",
                    error=f"Source path is not a file: {source}",
    ...
    if destination.exists() and not overwrite:
        return RunResult(
            ...
            data={
                SKILL_RESULT_KEY: build_skill_result(
                    summary="Rejected copy because destination exists",
    ...
    try:
        ...
        return RunResult(
            ok=True,
            output=f"Copied file to {destination}",
            data={
                SKILL_RESULT_KEY: build_skill_result(
                    summary=f"Copied file to {destination}",
                    machine_payload={
                    ...
                    operator_note="File copy completed in confined notes scope.",
