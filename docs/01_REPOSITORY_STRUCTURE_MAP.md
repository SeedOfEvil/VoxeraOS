...
- `app.py` — `FastAPI(title="Voxera Panel")`, composition root: shared health/queue wrappers, template env, dependency wiring, and route registration. Imports the operator Basic-auth guard and CSRF mutation guard from `auth_enforcement.py`, and the hygiene / queue mutation bridge entry points from `queue_mutation_bridge.py`.
