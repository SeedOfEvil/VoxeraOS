...
- `app.py` — `FastAPI(title="Voxera Panel")`, composition root: shared health/queue wrappers, template env, dependency wiring, and route registration. Imports the operator Basic-auth guard and CSRF mutation guard from `auth_enforcement.py`, the hygiene / queue mutation bridge entry points from `queue_mutation_bridge.py`, the panel security / health snapshot helpers from `security_health_helpers.py`, the job-detail payload builders from `job_detail_sections.py` / `job_presentation.py`, the health-view / formatting helpers from `health_view_helpers.py`, and the degraded-assistant bridge entry points from `degraded_assistant_bridge.py`.
...
- `queue_mutation_bridge.py` — hygiene / queue mutation bridge (fail-closed, atomic writes). Owns the narrow seam that turns panel mutation intents into actual queue/job writes and CLI subprocess invocations. Exposes `run_queue_hygiene_command(queue_root, args)`, `write_panel_mission_job(queue_root, *, prompt, approval_required)`, `write_queue_job(queue_root, payload)`, and `write_hygiene_result(queue_root, key, result, *, now_ms)`; `app.py` wraps these with thin `_*` shims that close over `_queue_root()` / `_now_ms` and passes them into the hygiene, home, and mission route registrations. Preserves the `source_lane=panel_queue_create` / `source_lane=panel_mission_prompt` queue-truth envelopes, the atomic `tmp_path.replace(final_path)` inbox writes, and the fail-closed `panel_hygiene_command_failed` audit path.
- `security_health_helpers.py` — panel security / health snapshot helper cluster (explicit-args, pure-ish). Owns the narrow wiring between the panel composition root and the health snapshot file: deriving the health-file queue root from the configured panel queue root (with the `VOXERA_HEALTH_PATH` isolation escape hatch), incrementing panel security counters, reading the panel security snapshot, and rendering the auth-setup banner. Exposes `health_queue_root(queue_root)`, `panel_security_counter_incr(queue_root, key, *, last_error)`, `panel_security_snapshot(queue_root)`, and `auth_setup_banner(settings)`; `app.py` wraps each with a thin `_*` shim that closes over `_queue_root()` / `_settings()` so existing route-registration callbacks (`health_queue_root=`, `panel_security_counter_incr=`, `panel_security_snapshot=`, `auth_setup_banner=`) and the `auth_enforcement` reach-back pattern (via `panel.app._health_queue_root` / `_panel_security_counter_incr`) continue to resolve to the same behavior.
...
- `routes_assistant.py` — `GET /assistant`, `POST /assistant/ask`, `GET /assistant/progress/{request_id}` (advisory assistant lane). Route handlers and registration; the degraded-assistant bridge logic lives in `degraded_assistant_bridge.py`.
- `degraded_assistant_bridge.py` — degraded-assistant bridge / messaging helper cluster (explicit-args, pure-ish). Owns the degraded-answer generation path: stall detection (`assistant_stalled_degraded_reason`), provider-tier brain factory (`create_panel_assistant_brain`), async/sync degraded-answer generation (`generate_degraded_assistant_answer_async`, `generate_degraded_assistant_answer`), and result persistence (`persist_degraded_assistant_result`). `app.py` aliases these as module-level `_assistant_stalled_degraded_reason` / `_create_panel_assistant_brain` / `_persist_degraded_assistant_result` and wraps the generation path with thin `_generate_degraded_assistant_answer` / `_generate_degraded_assistant_answer_async` shims that push monkeypatched `load_app_config` / `create_panel_assistant_brain` into the bridge module before calling. The module never imports `panel.app`.
...
- `helpers.py` — shared request/response helpers (coerce_int, request_value, etc.).
- `job_detail_sections.py` — job-detail shaping cluster (explicit-args, pure-ish). Owns the big `build_job_detail_payload(queue_root, job_id)` and `build_job_progress_payload(queue_root, job_id)` builders that power `GET /jobs/{job_id}` and `GET /jobs/{job_id}/progress`, plus the narrow `build_job_detail_sections(...)` composition helper that bundles the per-section slices from `job_presentation.py`. Also owns the small private loaders coupled to the builder (`_artifact_text`, `_safe_json`, `_load_actions`, `_read_generated_files`, `_payload_lineage`). `app.py` wraps each builder with a thin `_job_detail_payload` / `_job_progress_payload` shim that preserves the `(queue_root, job_id) -> dict` route-callback contract and passes them into `register_job_routes`. Queue-truth precedence (structured execution > state sidecar > raw bucket) and the 404-on-missing-job-and-artifacts semantics are preserved byte-for-byte. Prepares the ground for a future shared session-context / `vera_context` block in the panel job-detail surface.
- `job_presentation.py` — per-section shaping helpers for the job detail surface (`job_context_summary`, `operator_outcome_summary`, `policy_rationale_rows`, `evidence_summary_rows`, `why_stopped_rows`, `job_recent_timeline`, `job_artifact_inventory`) plus the tiny `job_artifact_flags(queue_root, job_id)` helper that powers the per-row artifact chips on `GET /jobs`.
- `health_view_helpers.py` — panel health-view / formatting helper cluster (explicit-args, pure). Owns the two read-only view builders that power the home page's Daemon Health widget and Performance Stats tab (`daemon_health_view(health)` and `performance_stats_view(queue, health)`) plus the five tiny formatting / history-line helpers those views depend on (`format_ts`, `format_ts_seconds`, `format_age`, `history_value`, `history_pair`). `app.py` wraps `daemon_health_view` / `performance_stats_view` / `format_ts` with thin `_daemon_health_view` / `_performance_stats_view` / `_format_ts` shims that preserve the `(health) -> dict` / `(queue, health) -> dict` route-callback contracts that `register_home_routes` and `register_automation_routes(format_ts_ms=_format_ts)` already expect. The module never imports `panel.app`.
