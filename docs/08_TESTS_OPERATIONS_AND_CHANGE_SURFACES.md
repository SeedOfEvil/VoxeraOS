...
### Time-aware context
- `test_time_context.py` — covers the time-context helpers in `src/voxera/vera/time_context.py`: current time context returns structured data; elapsed-time formatting for recent timestamps; time-until formatting for future timestamps; relative-day classification (today/yesterday/tomorrow); automation timing descriptions use human-readable phrasing; time question detection and direct answers from system clock; no fabricated execution history when timestamps are absent; prompt/instruction surfaces reflect time-aware capability; time context block for prompt injection; operator assistant system prompt includes time context; early exit dispatch handles time questions.
