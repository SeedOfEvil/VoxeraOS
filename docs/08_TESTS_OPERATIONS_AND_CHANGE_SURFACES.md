...
- `test_panel_voice_status.py` — operator-facing voice status panel page and JSON endpoint (18 tests): HTML page renders, foundation/STT/TTS sections present, disabled badges when foundation off, JSON link present, enabled+configured state shows available/backend names, no fake ready badges (strict: no badge-ok when disabled), config-not-runtime note present in subtitle, JSON endpoint ok/shape/stt-fields/tts-fields/disabled-state/configured-state/serializable, auth required for both HTML and JSON endpoints.
- `test_panel_voice_tts_generate.py` — operator-facing TTS generation flow (43 tests): form rendering (TTS Generation section, form fields, CSRF cookie, form action), success rendering (succeeded badge, audio path, backend, timing, request ID, preserved input, raw response), failure rendering (unavailable badge, error message, error class, no audio path on failure, failed status, empty/whitespace text validation), config-load failure (flags load crash shows unavailable not 500, synthesize_text not called when flags fail, JSON returns 500), no-fake-success invariants (no success when audio_path missing, no success when status not succeeded), canonical path invocation (calls `synthesize_text`, voice_id/language pass-through, empty optionals passed as None), auth/CSRF enforcement (HTML and JSON endpoints require auth and CSRF), JSON endpoint (success/failure shapes, empty text 400, serializability), existing voice status preservation (page renders, JSON works, auth enforced, no fake ready, config-not-runtime note, subtitle mentions generation and transcription).
- `test_panel_voice_stt_transcribe.py` — operator-facing STT transcription flow (41 tests): form rendering (STT Transcription section, form fields, form action, file-oriented description), success rendering (succeeded badge, transcript, detected language, backend, timing, request ID, preserved input path, raw response), failure rendering (unavailable badge, error message, error class, no transcript on failure, failed status, empty/whitespace path validation, unexpected pipeline exception renders error not 500), config-load failure (flags load crash shows unavailable not 500, transcribe_audio_file not called when flags fail, JSON returns 500), no-fake-success invariants (no success when transcript missing, no success when status not succeeded), canonical path invocation (calls `transcribe_audio_file`, language pass-through, empty optionals passed as None), auth/CSRF enforcement (HTML and JSON endpoints require auth and CSRF), JSON endpoint (success/failure shapes, empty path 400, serializability, unexpected exception returns 500), existing voice surface preservation (page renders, JSON works, TTS form still present, STT endpoint mentioned, no fake ready).
- `test_panel_voice_polish.py` — voice page UI/UX polish structure and CSS pins (36 tests): accessibility (skip-link, main landmark), navigation (page-nav with aria-label, section anchor links, zone ordering, nav hidden on error), hierarchy (config zone label, grid-2 status layout, card headings), section intros (TTS/STT/debug descriptions), empty-result states (TTS/STT empty messages, hidden when result present), TTS result rendering (voice-result class, ok/fail accents, result header, path-block, timing-group, raw-details), STT result rendering (voice-result class, ok/fail accents, transcript-block, timing-group, raw-details), form polish (action-form class, form-row, btn-primary), debug section (quieter card class), CSS class existence (18 voice-specific classes pinned in panel.css), error text styling (TTS/STT error-text class).
...
| Panel voice / TTS generation / STT transcription | `test_panel_voice_status.py`, `test_panel_voice_tts_generate.py`, `test_panel_voice_stt_transcribe.py`, `test_panel_voice_polish.py` |
