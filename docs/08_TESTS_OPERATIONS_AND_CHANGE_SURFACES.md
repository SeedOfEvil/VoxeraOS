...
- `test_voice_stt_adapter.py` — pins the STT backend adapter boundary and fail-soft transcription path: STTAdapterResult frozen immutability and defaults, NullSTTBackend truthful behavior (backend_name, unavailable result, protocol conformance), STTBackend protocol structural conformance (stub adapters satisfy protocol), transcribe_stt_request no-adapter path (unavailable + backend_missing), NullSTTBackend transcription path (failed + backend_missing), successful adapter (transcript, language, timing, whitespace normalization), unsupported input source (STTBackendUnsupportedError → unsupported + unsupported_source), backend exception (RuntimeError → failed + backend_error, never raises), adapter-reported error (passthrough error/error_class), empty/None transcript (failed + empty_audio), normalization consistency with input.py, all valid input sources succeed with working backend.
