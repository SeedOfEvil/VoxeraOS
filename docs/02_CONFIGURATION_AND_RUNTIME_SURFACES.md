...
- `voxera automation run-now <id>` — immediately process a single definition through the existing runner. Submits through the canonical inbox path — the queue remains the execution boundary.
