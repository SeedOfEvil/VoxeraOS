...
A second optional local backend is `MoonshineLocalBackend` (`voice/moonshine_backend.py`), which uses the official `moonshine-voice` PyPI package (ONNX-runtime-based, CPU-first) for audio file transcription. It satisfies the same structural `STTBackend` protocol, supports `audio_file` only, and lazy-loads its model on first `transcribe()`. The dependency is optional: install with `pip install voxera-os[moonshine]` (pulls `moonshine-voice` plus `av` for audio normalization). If the dependency is missing, the backend honestly reports `backend_missing`. Moonshine reads PCM WAV directly via its bundled pure-Python loader; non-WAV inputs (e.g. browser `audio/webm` from the Voice Workbench mic lane) are transparently transcoded to 16 kHz mono 16-bit PCM WAV through `voice/audio_normalize.py` (PyAV-backed) before hitting the loader, then the temp file is cleaned up in a `finally` block. Already-PCM-WAV inputs skip transcoding entirely so file-path uploads pay zero conversion overhead. Audio duration is derived from the decoded sample count; language detection is not exposed on the non-streaming path, so that field is `None` rather than fabricated. The panel mic-capture JS also prefers `audio/wav` directly via `MediaRecorder.isTypeSupported` when Moonshine is the active backend; on browsers that decline that hint (currently most desktop Chrome/Edge/Firefox default to `audio/webm`), the server-side transcode fallback handles it.

Backend selection is handled by `build_stt_backend(flags)` (`voice/stt_backend_factory.py`), which maps `VoiceFoundationFlags.voice_stt_backend` to the appropriate `STTBackend` implementation. When voice input is disabled, no backend is configured, or the backend identifier is unrecognized, it returns `NullSTTBackend`. When `voice_stt_backend` is `"whisper_local"`, it returns `WhisperLocalBackend`; `"moonshine_local"` returns `MoonshineLocalBackend`. The factory is the single point of backend selection logic, and its curated `STT_BACKEND_CHOICES` tuple is the source of truth for the panel Voice Options dropdown.
...
The recommended configuration path for voice is the setup wizard: `voxera setup` now includes a short voice step that asks whether to enable the foundation, STT, TTS, picks backends (`whisper_local` for STT; `piper_local` or `kokoro_local` for TTS), and — depending on the selected TTS backend — optionally accepts a Piper model name/path or Kokoro model / voices / voice ids. The wizard's answers are persisted to the runtime config JSON (`~/.config/voxera/config.json`) via `config.update_runtime_config(...)` under keys: `enable_voice_foundation`, `enable_voice_input`, `enable_voice_output`, `voice_stt_backend`, `voice_tts_backend`, `voice_tts_piper_model`, `voice_stt_whisper_model`, `voice_tts_kokoro_model`, `voice_tts_kokoro_voices`, `voice_tts_kokoro_voice`. Declining the foundation clears every voice key from the file so state cannot go stale against the most recent answer. Unrelated runtime keys (panel host/port, queue settings, etc.) are preserved.

The panel Voice Options form at `/voice/status` also lets the operator switch the STT backend between the curated `whisper_local` / `moonshine_local` choices and the TTS backend between `piper_local` / `kokoro_local`; the save route only updates fields the operator actually submitted, so a stale client that does not carry one of the selectors will not silently clear an existing selection. Unknown identifiers fail truthfully with a form-level error — nothing unrecognized is persisted.

The operator-selected local Whisper STT model id lives under `voice_stt_whisper_model`. The wizard does not prompt for it (the panel Voice Options form at `/voice/status` is the preferred surface); `None` / missing means the backend's own default (`base`) applies. The panel allow-list includes `base`, `small`, `medium`, `large-v3`, and `distil-large-v3`; the factory accepts any truthy string so env-only deployments can pin models outside the curated list. Values are the canonical faster-whisper identifiers — faster-whisper maps each to its CT2-compatible repo internally (e.g. `distil-large-v3` -> `Systran/faster-distil-whisper-large-v3`). Raw Hugging Face PyTorch repo ids like `distil-whisper/distil-large-v3` are NOT accepted (they download PyTorch weights that lack the CT2 `model.bin` the loader needs).

The operator-selected Moonshine STT model id lives under `voice_stt_moonshine_model`. `None` / missing means the backend's default (`moonshine/base`) applies. The panel allow-list includes `moonshine/tiny` (lowest latency) and `moonshine/base` (higher accuracy). Selection is only meaningful when `voice_stt_backend` is `moonshine_local`.
...
Environment variables for Moonshine backend configuration (loaded by `voice/moonshine_backend.py`):
- `VOXERA_VOICE_STT_MOONSHINE_MODEL` — Moonshine model id (default: `moonshine/base`; also read by `load_voice_foundation_flags` as an override of the runtime-config `voice_stt_moonshine_model` key).
...
Environment variables for Kokoro backend configuration (loaded by `voice/kokoro_backend.py` and mirrored into `VoiceFoundationFlags` so they also show up truthfully in `/voice/status`):
- `VOXERA_VOICE_TTS_KOKORO_MODEL` — absolute path to `kokoro-*.onnx` (required; no default path).
- `VOXERA_VOICE_TTS_KOKORO_VOICES` — absolute path to `voices-*.bin` (required; ships alongside the model).
- `VOXERA_VOICE_TTS_KOKORO_VOICE` — voice id to synthesize with (default: `af_sarah`).
- `VOXERA_VOICE_TTS_KOKORO_LANG` — language code (default: `en-us`; env-only, not surfaced through flags).
...
`KokoroLocalBackend` (`voice/kokoro_backend.py`) is the second real TTS backend, optional and operator-selectable. It uses `kokoro-onnx` for local, CPU-first speech synthesis and also supports `wav` output only. The dependency is optional: install with `pip install voxera-os[kokoro]`. Unlike Piper, Kokoro has no cross-deployment default model path, so missing / unset `voice_tts_kokoro_model` or `voice_tts_kokoro_voices` is surfaced as a truthful `backend_missing` error with a concrete operator-facing message; the `tts_dependency.kokoro_model` sub-block in `/voice/status.json` reports the configured paths, file-existence flags, and effective voice id. Session loading is lazy, mirroring Piper.

Backend selection is handled by `build_tts_backend(flags)` (`voice/tts_backend_factory.py`), which maps `VoiceFoundationFlags.voice_tts_backend` to the appropriate `TTSBackend` implementation. When voice output is disabled, no backend is configured, or the backend identifier is unrecognized, it returns `NullTTSBackend`. When `voice_tts_backend` is `"piper_local"`, it returns `PiperLocalBackend`. When `"kokoro_local"`, it returns `KokoroLocalBackend`. The factory is the single point of backend selection logic, and its curated `TTS_BACKEND_CHOICES` tuple is shared by the panel Voice Options form and the setup wizard so adding a new backend flows through both surfaces from one source of truth.
