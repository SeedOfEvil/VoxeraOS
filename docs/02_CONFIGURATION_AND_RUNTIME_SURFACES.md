...
The panel Voice Options form at `/voice/status` also lets the operator switch the TTS backend between the curated `piper_local` / `kokoro_local` choices; the save route only updates fields the operator actually submitted, so a stale client that does not carry the TTS selector will not silently clear an existing selection.
