...
The TTS request/response protocol (`voice/tts_protocol.py`) defines the symmetric contract shapes for text-to-speech interactions. This is the protocol definition layer only — no synthesis backend, adapter, or factory is wired yet. The key distinction from the STT protocol: the response output is an artifact path (`audio_path`), not a transcript string. See `09_CORE_OBJECTS_AND_SCHEMA_REFERENCE.md` for the full TTS protocol schema shapes.
