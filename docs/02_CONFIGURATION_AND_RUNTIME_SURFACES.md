...
A second optional local backend is `MoonshineLocalBackend` (`voice/moonshine_backend.py`), which uses `moonshine-onnx` (ONNX-runtime-based, CPU-first) for audio file transcription. It satisfies the same structural `STTBackend` protocol, supports `audio_file` only, and lazy-loads its model on first `transcribe()`. The dependency is optional: install with `pip install voxera-os[moonshine]`. If the dependency is missing, the backend honestly reports `backend_missing`. Moonshine does not expose audio duration or language detection on the ONNX path, so those fields are returned as `None` rather than fabricated.
