...
A second optional local backend is `MoonshineLocalBackend` (`voice/moonshine_backend.py`), which uses the official `moonshine-voice` PyPI package (ONNX-runtime-based, CPU-first) for audio file transcription. It satisfies the same structural `STTBackend` protocol, supports `audio_file` only, and lazy-loads its model on first `transcribe()`. The dependency is optional: install with `pip install voxera-os[moonshine]`. If the dependency is missing, the backend honestly reports `backend_missing`. Moonshine reads PCM WAV files directly (16 / 24 / 32-bit) via its bundled pure-Python loader — no FFmpeg required; non-WAV inputs surface a truthful `backend_error` ("PCM WAV required"). Audio duration is derived from the decoded sample count; language detection is not exposed on the non-streaming path, so that field is `None` rather than fabricated.
