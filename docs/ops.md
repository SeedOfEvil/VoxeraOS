...
- `src/voxera/vera/service.py` remains the top-level Vera orchestrator (LLM reply, preview builder, linked completion delivery). Session state helpers live in `session_store.py`; callers import from `session_store` directly rather than through `service.py` re-exports.
...
Vera session persistence/state ownership lives in `src/voxera/vera/session_store.py`. Callers import session helpers directly from `session_store`; `service.py` no longer re-exports session functions. Prefer adding or updating session read/write helpers in `session_store.py` first when working on future Vera modularization PRs.
