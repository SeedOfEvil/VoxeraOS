...
- `src/voxera/vera/handoff.py` is **deprecated and empty**. Import directly from the source modules:
...
- Panel route ownership is split across `src/voxera/panel/routes_*.py`; prefer extending the route-family module first instead of adding new route behavior directly to `panel/app.py` unless the change is cross-cutting wiring/shared helper work. The automation dashboard (`routes_automations.py`) provides operator visibility and control over saved automation definitions — it does not author definitions or bypass the queue. Run-now is queue-submitting only.
...
Vera shared session context lifecycle freshness is owned by `src/voxera/vera/context_lifecycle.py`. This module defines explicit, named lifecycle update functions for each workflow event (preview created/revised/renamed/cleared, handoff, completion ingestion, review, follow-up preparation, session clear). Prefer adding new lifecycle update points in `context_lifecycle.py` rather than scattering inline `update_session_context` calls in `app.py`.

Vera weather quick-flow orchestration lives in `src/voxera/vera/weather_flow.py`. `service.py` imports from `weather_flow` directly — no compatibility aliases remain. Prefer extending `weather_flow.py` for weather-question detection, missing-location handling, follow-up continuity, and fail-closed weather-lane behavior instead of re-growing those branches inside `service.py`. Tests that need to control weather behavior through `generate_vera_reply` should patch the imported name in `vera.service` (e.g. `monkeypatch.setattr(vera_service, "weather_context_has_pending_lookup", ...)`); tests that exercise weather helpers directly should import from `vera.weather_flow`.

Vera saveable assistant artifact selection now lives in `src/voxera/vera/saveable_artifacts.py`. Prefer extending `saveable_artifacts.py` for meaningful-content selection, courtesy/control filtering, and recent `"save that"` target resolution.

Vera active preview draft revision interpretation now lives in `src/voxera/vera/draft_revision.py`, with `src/voxera/vera/handoff.py` delegating rename/path/content refinement parsing into that module while keeping the handoff-side entrypoint stable. Prefer extending `draft_revision.py` for active preview rename/path/content follow-up behavior instead of re-growing that seam inside `handoff.py`. `draft_revision.py` now also supports authored-content transformation patterns (concise, checklist, tone shifts) consumed by both the active-preview refinement path and the session-context-aware follow-up path in `preview_drafting.py`.

Vera session-context-aware authored follow-up resolution lives in `src/voxera/vera/preview_drafting.py`. When no active preview exists but session context has `active_draft_ref`, natural follow-up transformation requests resolve against the most recent saveable assistant artifact. Fail-closed when context is missing, ambiguous, or empty. `maybe_draft_job_payload` accepts `session_context` for this path.

Vera preview submission and handoff normalization live in `src/voxera/vera/preview_submission.py`. Callers import directly from that module. Prefer extending `preview_submission.py` for submit/current-preview behavior instead of re-growing that seam inside `app.py`.
...
Naming-mutation rule: valid rename/name-note turns must update canonical preview `write_file.path` and return explicit destination confirmation text; ambiguous naming phrases fail closed with an unchanged draft. When the hidden compiler returns an unchanged preview for a clear rename intent, the deterministic rename-mutation path is used as a fallback so the rename is never silently lost.
Content-purity rule: canonical `write_file.content` must contain only authored body content, never draft-management/readiness narration. Wrapper replies like “I added ... to the file content” are not saveable as body text; if they quote authored body text, only the quoted body is eligible for preview content. Summary-type generate+save flows must strip “You can review the content...” and “Please review the content...” helper prefixes.
Single-turn generate+save rule: clear prompts that both generate and save (for example “write a short poem ... and save it as ...”) may stage a preview with empty body first, then bind same-turn authored reply content as canonical `write_file.content`; do not require prior assistant artifact history for those cases.
Submit-intent strictness rule: typo-like near-submit phrases (for example “send iit”) must fail closed with an explicit “did not submit” message. Vera must never imply submission happened when no real queue handoff occurred. Near-miss detection runs before the message reaches the LLM to prevent conversational overclaiming.

Vera deterministic preview drafting lives in `src/voxera/vera/preview_drafting.py`. Prefer extending `preview_drafting.py` for narrow action-preview drafting, diagnostics preview shaping, note/file drafting, and contextual save-by-reference glue.

Vera automation preview drafting, revision, and submit-to-store lives in `src/voxera/vera/automation_preview.py`. Vera can now draft automation definitions conversationally ("every hour, run system_inspect") and save them as durable `AutomationDefinition` records. Submit saves the definition — it does NOT emit a queue job. Execution remains through the automation runner and queue. Supported authoring trigger kinds: `delay`, `recurring_interval`, `once_at`. The `recurring_cron` and `watch_path` trigger kinds are not offered during Vera authoring because the runner does not support them yet. Prefer extending `automation_preview.py` for automation-specific drafting, revision, and submit behavior instead of mixing it into the normal preview flows.

Vera conversational lifecycle management for saved automation definitions lives in `src/voxera/vera/automation_lifecycle.py`. Vera can now manage saved automations conversationally: "show me that automation", "disable it", "enable it again", "delete that automation", "run it now", "did it run?", "show me the history". Reference resolution uses session context (active_topic), explicit id, title match, or single-definition fallback — ambiguous references fail closed with a clarification request. Enable/disable mutate the saved definition via the existing store semantics. Delete removes the definition but preserves history. Run-now uses the existing automation runner force path — it still goes through the canonical runner → inbox → queue path. Vera does not execute payloads directly. The queue remains the execution boundary. Prefer extending `automation_lifecycle.py` for saved-automation management actions instead of mixing them into the authoring or preview flows.

Checklist/planning conversational turns are intentionally deterministic in-chat artifacts: `src/voxera/vera_web/conversational_checklist.py` owns checklist parsing/rendering/sanitization helpers, `src/voxera/vera_web/execution_mode.py` owns non-I/O execution-mode classifier predicates/helpers, and `src/voxera/vera_web/preview_content_binding.py` owns low-risk preview-body/content-binding purity helpers. `src/voxera/vera_web/app.py` keeps route-level orchestration and truth-critical preview/submit/session-state boundaries. Source priority is explicit user-provided checklist items first, then usable extracted model/JSON items, then generic fallback only if extraction fails. Explicit save/write intent remains the authoritative override into governed preview behavior.

Post-LLM draft content extraction and preview binding derivation now lives in `src/voxera/vera_web/draft_content_binding.py`, with `src/voxera/vera_web/app.py` delegating `extract_reply_drafts` and `resolve_draft_content_binding` into that module while keeping all final `write_session_preview`/`write_session_handoff_state` calls in `app.py`. Prefer extending `draft_content_binding.py` for code/writing draft extraction, draft-to-preview injection, generation content binding, and create-and-save fallback behavior instead of re-growing those branches inside `app.py`.

Post-guardrail assistant reply assembly now lives in `src/voxera/vera_web/response_shaping.py`, with `src/voxera/vera_web/app.py` delegating `derive_preview_has_content`, `guardrail_false_preview_claim`, `should_clear_stale_preview`, and `assemble_assistant_reply` into that module while keeping all I/O (session reads/writes, `_guardrail_submission_claim`, `append_session_turn`, `_render_page`) in `app.py`. Prefer extending `response_shaping.py` for pure reply text selection, preview-existence claim guardrailing, control-turn suppression, and fail-closed messaging instead of re-growing those branches inside `app.py`.

Early-exit intent handler dispatch now lives in `src/voxera/vera_web/chat_early_exit_dispatch.py`. `app.py` calls `dispatch_early_exit_intent(...)` to evaluate the coherent cluster of special-intent / short-circuit conditions (diagnostics refusal, job review, follow-up preview, investigation derived-save/compare/summary/expand/save, near-miss submit blocking) before the LLM path. The function returns `EarlyExitResult` with write instructions and assistant text; `app.py` then performs all session writes (`write_session_preview`, `write_session_handoff_state`, `write_session_derived_investigation_output`), appends the turn, and renders the page. Submit/handoff truth paths, the weather-context LLM lookup, and the blocked-file guard remain in `app.py`. Prefer extending `chat_early_exit_dispatch.py` for new self-contained early-return intent branches that produce a structured response without touching final session state writes.
...
- Panel shared wiring/security and canonical job-detail assembly ownership remain in `src/voxera/panel/app.py`. Extracted low-risk helpers now include presentation/status row builders in `src/voxera/panel/job_presentation.py`, job-detail section assembly helpers in `src/voxera/panel/job_detail_sections.py`, and panel auth-state storage/lockout bookkeeping helpers in `src/voxera/panel/auth_state_store.py`; route-family ownership lives in `src/voxera/panel/routes_*.py` modules:
...
- `src/voxera/panel/job_presentation.py` contains low-risk row/presentation helper builders used by `app.py` (operator outcome labels, policy/evidence rows, why-stopped rows, and related display shaping).
- `src/voxera/panel/job_detail_sections.py` contains low-risk job-detail section assembly helpers used by `app.py` (operator summary, policy rationale, evidence summary, why-stopped, and recent timeline) built from already-loaded canonical data.
