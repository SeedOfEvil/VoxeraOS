...
Checklist/planning conversational turns are intentionally deterministic in-chat artifacts: `src/voxera/vera_web/conversational_checklist.py` owns checklist parsing/rendering/sanitization helpers, and `src/voxera/vera_web/execution_mode.py` owns non-I/O execution-mode classifier predicates/helpers, while `src/voxera/vera_web/app.py` keeps route-level orchestration and truth-critical preview/submit boundaries. Source priority is explicit user-provided checklist items first, then usable extracted model/JSON items, then generic fallback only if extraction fails. Explicit save/write intent remains the authoritative override into governed preview behavior.
