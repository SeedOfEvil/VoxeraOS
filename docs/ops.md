...
- Use **Run prune (dry-run)** to execute `voxera queue prune --dry-run --json` via local CLI invocation; this is report-only and never deletes data from this panel path.
