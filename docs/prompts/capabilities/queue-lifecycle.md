...
- Auto-surfacing lane (current slice): only linked completions classified as `read_only_success`, `mutating_success`, `approval_blocked`, or `failed` may be proactively surfaced in chat; mutating success must pass terminality guards (no pending/delegated downstream child work); other classes remain manual until expanded.
