...
`vera/session_store.py` persists a bounded `shared_context` dict alongside turns, preview, handoff, and other session fields. It tracks workflow-continuity references (active draft, active preview, last submitted/completed/reviewed job, active topic, ambiguity flags). `vera/context_lifecycle.py` defines explicit lifecycle update functions that fire at each workflow event: preview created/revised/renamed, preview cleared, handoff/submit, linked job registration, completion ingestion, review, follow-up/revision/save-follow-up preparation, and session clear. Context is subordinate to preview, queue, and artifact/evidence truth — if context conflicts with canonical truth, canonical truth wins.
