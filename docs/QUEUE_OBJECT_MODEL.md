...
- Shared session context (additive): Vera maintains a bounded `shared_context` object inside each session that tracks workflow-continuity references (active draft, active preview, last submitted/completed/reviewed job, last saved file, active topic, ambiguity flags). This context is a continuity aid subordinate to preview, queue, and artifact/evidence truth. Explicit lifecycle update points (`vera/context_lifecycle.py`) keep context fresh across preview create/revise/rename/clear, handoff, completion ingestion, review, follow-up preparation, and session clear. If session context conflicts with canonical truth surfaces, canonical truth wins. If continuity is ambiguous, the system fails closed.
