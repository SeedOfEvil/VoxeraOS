...
Key areas:
- Shared operational session context between Vera and VoxeraOS (foundation shipped: bounded session context model with lifecycle hooks; session-scoped reference resolution shipped: drafts, files, job results, continuations; lifecycle freshness shipped: context stays current across preview/handoff/completion/review/follow-up events)
