...
- Draft revision interpretation for those rename/path/content follow-ups now lives in `src/voxera/vera/draft_revision.py`; `handoff.py` delegates to that module so the conversational refinement seam has one explicit owner without changing preview or queue semantics.
