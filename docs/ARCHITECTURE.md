...
- **First-turn previewable automation polish (`vera_web/app.py`):** the blanket refusal text is exported as `BLANKET_PREVIEW_REFUSAL_TEXT` from `response_shaping.py`. Immediately after `guardrail_false_preview_claim`, when (a) the guarded answer is exactly that blanket refusal, (b) no preview is active, and (c) the user message matches the structural detector `_looks_like_previewable_automation_intent` (intent verb + file/folder subject + action-on-arrival hint), the visible reply is replaced with `_PREVIEWABLE_AUTOMATION_CLARIFICATION_REPLY` — a focused question asking for source folder, destination folder, and action-on-arrival. **No preview is materialized**; this is a routing-only substitution that prevents over-conservative first-turn refusals for clearly previewable automation requests without weakening the trust model. The detector requires three independent structural signals so weather, writing, simple file ops, and informational requests fail closed.
...
- **Session-aware authored drafting follow-ups:** When no active preview exists but session context has `active_draft_ref`, natural follow-up transformation requests ("make that more concise", "turn that into a checklist", "make that more operator-facing") resolve against the most recent saveable assistant artifact via `_resolve_authored_followup_from_session_context` in `vera/preview_drafting.py`. Fail-closed when session context has no active draft ref, no artifacts, or empty artifact content. This makes authored drafting flows feel session-continuous without promoting session context into a truth surface.
- **Authored-content transformation patterns:** `refined_content_from_active_preview` in `vera/draft_revision.py` supports: concise compression, checklist/bullet-list conversion, operator-facing/user-facing tone shifts, and same-tone preservation. These work both in the active-preview refinement path and the session-context-aware follow-up path.
