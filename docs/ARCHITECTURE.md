...
- Ordinary weather/current-condition questions use a dedicated Brave-backed quick-weather lane first; they do not fall through to freeform conversational guessing.
- The quick-weather lane is truthful-by-construction: if location is missing Vera asks for it, if a Brave-backed bounded extraction succeeds Vera returns a concise synthesized weather answer, and if lookup fails or source evidence is too weak Vera refuses to invent live conditions.
