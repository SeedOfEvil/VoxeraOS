...
- `files.read_text`, `files.write_text`, `files.list_dir`, `files.copy_file`, `files.move_file`, `files.mkdir`, `files.exists`, `files.stat`, and `files.delete_file` share centralized path-boundary enforcement in `src/voxera/skills/path_boundaries.py`.
