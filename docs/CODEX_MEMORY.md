...
- **Tests**: 36 new tests in `tests/test_vera_preview_materialization.py` covering: `_is_empty_code_preview_shell` (7 unit), `_CODE_DRAFT_RECOVERY_RE` pattern matching (11 parametrized), `_recover_code_draft_from_history` (4 unit), post-clarification creates real preview (integration), post-clarification go-ahead submits (integration), no false preview claim without real preview (integration), honest fail-closed when no code generated (integration), recovery via "please prepare that script" (integration), workspace-rooted script path (integration).
- **Validation**: `ruff format --check .`, `ruff check .`, `mypy src/voxera`, `pytest -q` — all pass (2497+ passed, 2 skipped). No runtime behavior change to unrelated flows.
- **Import guidance**: post-clarification code draft detection is internal to `app.py`; no new public exports. `_is_empty_code_preview_shell` and `_recover_code_draft_from_history` are app-internal helpers.
