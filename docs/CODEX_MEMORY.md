...
- **What shipped**: CSS + light JS only, no backend or route changes.
  ...
  - **Cleanup**: removed unused .responding-indicator CSS (was defined but never present in HTML, replaced by .thinking-indicator). Removed gratuitous `display: flex; flex-direction: column` on `.thread` (block flow already stacks children; adding flex risked subtle scroll/overflow differences). Eliminated JS variable shadowing in submit handler (redundant `const thread` re-query). Consolidated test CSS fixture. Moved `bounce-dot` keyframes under the thinking-indicator section comment.
