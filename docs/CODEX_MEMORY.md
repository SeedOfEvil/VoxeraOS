## 2026-03-23 — PR #229 — fix(vera): remove preview/queue behavior from conversational checklist/planning and enforce strict preview truth

- **Root cause:** Three classes of preview/submission truth violation for answer-first checklist/planning turns:
  1. False preview-pane claims ("You can see the draft in the preview pane") passed through because the guardrail was entirely skipped for answer-first turns.
  2. False submission claims ("I've submitted that checklist to the queue") passed through because `_guardrail_submission_claim` ran before the answer-first branching and replaced the entire answer with a refusal — or the LLM's submission language didn't match the narrow phrase list.
  3. JSON/VoxeraOS payload blobs leaked into conversational answers.
- **Fix — comprehensive conversational answer sanitizer:** Rewrote `_sanitize_false_preview_claims_from_answer()` as a three-phase sanitizer: (1) strip fenced JSON blocks containing VoxeraOS payloads, (2) strip lines containing preview-update claims (24+ phrases), (3) strip lines containing submission claims (18+ phrases). All content that is NOT a false claim is preserved.
- **Fix — guardrail restructure:** For `conversational_answer_first_turn`, the comprehensive sanitizer replaces BOTH the submission guardrail and the preview guardrail. Heavy guardrails (which replace the entire answer) only run on non-answer-first turns. This prevents useful checklist content from being destroyed.
- **Fix — multi-turn continuation:** Session-persisted `conversational_planning_active` flag keeps follow-up detail-providing turns in the answer-first lane.
- **Product concept enforced — conversational artifacts vs governed actions:** Checklists, plans, brainstorms, and summaries are ephemeral conversational artifacts that live in chat. They are not system actions and are not governed by default. They can be saved later via "save that". Preview/queue language must not appear unless a real preview/job exists.
- **Files changed:** `src/voxera/vera/session_store.py` (session field), `src/voxera/vera/service.py` (re-export), `src/voxera/vera_web/app.py` (comprehensive sanitizer + guardrail restructure + continuation flag), `tests/test_vera_session_characterization.py` (11 new tests), `docs/ARCHITECTURE.md`, `docs/CODEX_MEMORY.md`.
- **Tests added:** (1) preview-pane claim sanitized, (2) clean answer passes through, (3) multi-turn clarification stays answer-first, (4) multi-turn then save-that creates preview, (5) send-it without save is truthful, (6) continuation clears on save intent, (7) submission claim sanitized, (8) preview-update claim sanitized ("I've prepared a draft"), (9) JSON blob sanitized, (10) send-it after checklist without save is truthful (no fake submission), (11) checklist output is plain text not JSON.
- Validation: `ruff format --check .`, `ruff check .`, `mypy src/voxera`, `pytest -q` (1419 passed), `make security-check`, `make golden-check`, `make validation-check`, `make merge-readiness-check` — all pass.
- **Remaining limitations:** The phrase-based sanitizer is bounded — novel LLM phrasings may not be caught. Continuation flag persists for one follow-up turn only. Very long multi-turn planning chains (3+ turns) re-enter answer-first only if the user's message matches a planning keyword or the immediate prior turn was answer-first.
