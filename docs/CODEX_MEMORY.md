## 2026-04-24 — fix(vera): make active draft content updates mutate preview truthfully

- **Regression**: after PR #365, a remaining trust gap — the user could ask "add 10 more jokes to the list" against an active write_file preview, and Vera would reply "I've added 20 additional jokes… This brings the total list to 30." while the preview payload stayed unchanged. "update the preview" then replied "The current draft is still in the preview, unchanged." and another "add 10 more jokees" triggered the same false-success claim.
- **Root cause**: the append/expand intent class ("add N more X", "append N more", "continue the list", "expand it", "make it longer") had no deterministic binding path. The builder's `interpret_active_preview_draft_revision` routed these into `content_refinement_intent` but `extract_content_refinement` / `refined_content_from_active_preview` produced no output (none of the transform patterns matched). The builder returned None, the LLM was called as a normal turn, the LLM generated a confident "I've added N jokes" reply, and `draft_content_binding.py` had no path that would take the LLM's text and append it to the existing preview content. `response_shaping.py` had no filter for the "added N" / "this brings the total to N" false-claim shape, so the LLM reply leaked verbatim.
- **Fix (src/voxera/vera/draft_revision.py)**: new `is_active_preview_content_expand_request(message)` and `extract_expand_requested_count(message)` helpers. Detection covers digit counts, word counts ("ten"), typo variants ("jokees"), filler words ("additional", "new", "extra"), and the continue/expand/make-it-longer family. Ambiguous bare adds ("add it", "add that", "add a comment") are explicitly excluded.
- **Fix (src/voxera/vera_web/draft_content_binding.py)**: new append-style binding path that runs when an expand intent matches an existing refinable prose preview. On success, appends the LLM's `reply_text_draft` to the existing `write_file.content` (preserving path and mode) and sets `builder_payload` so the preview pane reflects the new body. Best-effort dedupe: if the LLM replied with the full new content (existing body + additions) rather than just additions, treats it as REPLACE to avoid doubling. Wrapper/status narration is rejected via `looks_like_non_authored_assistant_message`. On failure (empty or wrapper reply), sets `generation_content_refresh_failed_closed = True` so response shaping can surface an honest "draft unchanged" reply.
- **Fix (src/voxera/vera_web/conversational_checklist.py)**: `looks_like_preview_update_claim` now catches append/expand false-success phrases — explicit substrings ("added to the list/content/draft/note/file/preview", "appended to …", "expanded the …", "extended the …") plus a regex for numeric forms ("added 20 additional jokes", "appended 5 more bullets", "this brings the total to 30"). Added filler-adjective support (additional/new/extra/further/another) so the LLM's natural phrasings are caught.
- **Fix (src/voxera/vera_web/response_shaping.py)**: when `generation_content_refresh_failed_closed` fires AND the assistant text asserts a false update claim, replace the text entirely with "I could not safely update the active preview, so I left it unchanged." instead of appending the honest note after the false claim — the old append-only path produced contradictory mixed replies ("I added 20 jokes. I left the draft unchanged.").
- **Tests**: `tests/test_vera_preview_content_truth.py` grew from 68 → 90 tests. New sections cover: expand detection (digit, word count, typo, continue-the-list, expand-it, make-it-longer, no-match for ambiguous adds); `extract_expand_requested_count` (digit → value, word → value, no count → None, oversized → None); append binding (path preserved, existing body preserved, wrapper rejected, LLM-full-reply dedupe); response-shaping false-claim replacement; and `looks_like_preview_update_claim` positive/negative regressions.
- **Scope preserved**: no changes to queue, daemon, panel, STT, or TTS. Preview ownership, submit semantics, empty-content guard, inspection handler, lane precedence, and session-context-as-continuity rules are all unchanged. This fix is additive: a new binding path for a previously unhandled intent class, plus a targeted response-shaping replacement for the false-claim shape it tends to produce.
...
- **Test coverage**: `tests/test_panel_voice_stt_transcribe.py` (41 tests): form rendering (4), success rendering (8), failure rendering (7 — includes unexpected pipeline exception), config-load failure (3), no-fake-success invariants (2), canonical path invocation (3), auth/CSRF enforcement (4), JSON endpoint (5 — includes unexpected exception 500), existing voice surface preservation (5).
...
- **Files touched**: `src/voxera/panel/routes_voice.py` (updated), `src/voxera/panel/templates/voice.html` (updated), `tests/test_panel_voice_stt_transcribe.py` (new, 41 tests), `tests/test_panel_voice_tts_generate.py` (updated — subtitle test), `tests/test_panel_contract_snapshot.py` (updated — 2 new routes), `docs/CODEX_MEMORY.md`, `docs/08_TESTS_OPERATIONS_AND_CHANGE_SURFACES.md`, `docs/02_CONFIGURATION_AND_RUNTIME_SURFACES.md`.
...
- **Test coverage**: `tests/test_panel_voice_tts_generate.py` (43 tests): form rendering (4), success rendering (7), failure rendering (6), config-load failure (3), no-fake-success invariants (2), canonical path invocation (5), auth/CSRF enforcement (4), JSON endpoint (4), existing voice status preservation (8).
- **Docs updated**: `docs/CODEX_MEMORY.md`, `docs/08_TESTS_OPERATIONS_AND_CHANGE_SURFACES.md`, `docs/02_CONFIGURATION_AND_RUNTIME_SURFACES.md`.
- **Files touched**: `src/voxera/panel/routes_voice.py` (updated), `src/voxera/panel/templates/voice.html` (updated), `src/voxera/panel/app.py` (updated), `tests/test_panel_voice_tts_generate.py` (new, 43 tests), `tests/test_panel_contract_snapshot.py` (updated — 2 new routes), `docs/CODEX_MEMORY.md`, `docs/08_TESTS_OPERATIONS_AND_CHANGE_SURFACES.md`, `docs/02_CONFIGURATION_AND_RUNTIME_SURFACES.md`.
...
- **Changes**:
  ...
  - Add focused pipeline tests (`tests/test_voice_tts_pipeline.py`, 50 tests).
...
- **Updated module — `src/voxera/voice/output.py`**:
  ...
  - `voice_output_status(flags)`: updated stale `"voice_output_placeholder_only"` reason to `"voice_output_ready"` now that factory is wired.
- **Tests — `tests/test_voice_tts_pipeline.py`** (50 tests): factory null cases (6), factory Piper selection (3), factory error messages (3), factory constant (1), pipeline unconfigured paths (4), pipeline success paths (2), pipeline failure paths (2), canonical path threading (7 — voice_id, language, session_id, speed, output_format pass-through + empty text ValueError), backend name propagation (2), export surface (3), config-driven integration (4), response uniqueness (1), pre-built backend reuse (3), async entry point (6), no-fake-synthesis invariants (2).
...
- **Files touched**: `src/voxera/voice/tts_backend_factory.py` (new), `src/voxera/voice/output.py` (updated), `src/voxera/voice/__init__.py` (updated exports), `tests/test_voice_tts_pipeline.py` (new, 50 tests), `docs/CODEX_MEMORY.md`, `docs/02_CONFIGURATION_AND_RUNTIME_SURFACES.md`, `docs/08_TESTS_OPERATIONS_AND_CHANGE_SURFACES.md`, `docs/09_CORE_OBJECTS_AND_SCHEMA_REFERENCE.md`.
...
- **Scope (deliberately bounded)**:
  ...
  - Add focused backend tests (`tests/test_voice_piper_backend.py`, 45 tests).
...
- **Files touched**: `src/voxera/voice/piper_backend.py` (new), `src/voxera/voice/__init__.py` (updated exports), `pyproject.toml` (piper optional dep), `tests/test_voice_piper_backend.py` (new, 45 tests), `docs/CODEX_MEMORY.md`, `docs/02_CONFIGURATION_AND_RUNTIME_SURFACES.md`, `docs/08_TESTS_OPERATIONS_AND_CHANGE_SURFACES.md`, `docs/09_CORE_OBJECTS_AND_SCHEMA_REFERENCE.md`.
...
- **Test coverage**: `tests/test_voice_tts_protocol.py` — request construction (all valid formats, case normalization, unknown format rejection, empty text rejection, speed clamping, auto-generated ids, explicit ids/timestamps, frozen immutability), success response shape (audio_path stripping, empty audio_path → None, timing fields), audio_path semantics (present on success, None on failure/unavailable), failure response shape (error + error_class carriage, unsupported status), unavailable response convenience builder (required error_class, backend passthrough), nullable fields on failure/unavailable, fail-closed status normalization (unknown/empty/None/uppercase status → unavailable or correct lowercase), error_class passthrough policy (arbitrary strings accepted, all six canonical constants pass through), serialization helpers (request/response as_dict roundtrip, field-count guard, JSON serializability), schema version correctness.
- **Docs updated**: `docs/09_CORE_OBJECTS_AND_SCHEMA_REFERENCE.md` (TTS protocol schema shapes), `docs/08_TESTS_OPERATIONS_AND_CHANGE_SURFACES.md` (test listing), `docs/02_CONFIGURATION_AND_RUNTIME_SURFACES.md` (TTS protocol note in voice subsystem section), `docs/CODEX_MEMORY.md`.
...
- **What shipped**: template + CSS + focused tests only. No route, backend payload, queue/artifact truth, or action semantic changes.
  - **Semantic landmarks**: `<main id="main-content">` landmark added to all four key templates. Skip-to-content link (`.skip-link`) added to all pages for keyboard users.
  - **ARIA tab pattern**: home page tab bar converted from `<nav>` to proper `role="tablist"` / `role="tab"` / `role="tabpanel"` with `aria-selected`, `aria-controls`, `aria-labelledby`, and keyboard arrow-key navigation (ArrowLeft/Right/Home/End).
  - **ARIA labels**: `aria-label` added to all approval/deny/cancel/retry/delete action buttons across home, jobs, and job detail templates. `aria-label="Panel sections"` on page nav. `aria-label="Job filter"` on jobs filter form. `role="group" aria-label="Queue controls"` on queue pause/resume. `role="group" aria-label="Approval actions"` and `role="group" aria-label="Job actions"` on job detail action bars. `aria-hidden="true"` on decorative nav separators.
  - **Alert roles**: `role="alert"` on error/warning alerts, `role="status"` on success/queue-status badges across home and jobs templates.
  - **Focus visibility**: `:focus-visible` styles for `.btn`, `.tab-btn`, `a`, and `.collapsible summary` using consistent accent-color outline + soft box-shadow ring. Tab buttons gain `z-index` and border highlight on focus.
  - **Responsive improvements**: new tablet breakpoint at 768px — page header stacks vertically, tabs scroll horizontally, jobs filter form collapses from 4-column to 2-column then 1-column at 540px, `detail-grid` collapses to single-column, `.grid-3` transitions through 2-column (768px) to 1-column (540px) instead of jumping directly. At 540px: container padding tightens, button tap targets increase, page nav compresses, pre blocks gain `overflow-x: auto`. KPI grid adapts from 5→3→2 columns across breakpoints.
  - **Status text reinforcement**: KPI cards show `(action needed)` when pending_approvals > 0 and `(attention)` when failed > 0, alongside the existing color styling. Artifact pills on jobs page gain check/cross marks (✓/✗) alongside color. Evidence summary badges in job detail gain check/cross marks (✓ present / ✗ not present).
  - **Cleanup**: removed inline `style` on jobs filter form (replaced with `.jobs-filter-form` / `.jobs-filter-row` classes). Refined `.grid-3` breakpoint cascade (no longer collapses to 1fr at 980px; transitions through 768px→540px instead). Review pass: merged duplicate `.tab-btn:focus-visible` rules, removed dead `.badge:focus-visible` (badges are not focusable), removed redundant `role="banner"` on job detail `<header>` (implicit from element), removed doubled `aria-label` on job detail actions `<section>` (kept only on inner `role="group"`), removed redundant 768px `.dash-kpi-grid` override (already set at 980px), tightened two CSS-only test assertions to check specific breakpoint content. Second review pass: moved `<main>` in home.html to include the tab bar and alerts (skip-link no longer jumps past the tabs), added roving tabindex on tab buttons (`tabindex="0"` on active, `tabindex="-1"` on inactive, managed by JS on switch), moved `<main>` in vera.html above the error alert, replaced hardcoded `8px 14px` mobile button padding with `var(--space-2) var(--space-4)` token form, removed `role="status"` and `role="alert"` from server-rendered elements (live-region roles are for dynamically injected content, not page-load markup).
- **Tests**: `test_panel_accessibility.py` (26 tests) — skip-link and `<main>` landmark presence (home, jobs, job detail), page-nav `aria-label`, nav separator `aria-hidden`, ARIA tablist/tab/tabpanel pattern with `aria-selected`/`aria-controls`/`aria-labelledby`/roving `tabindex`, keyboard navigation script presence, queue status badge text, approval/retry/cancel button `aria-label` on home and jobs, queue controls `role="group"`, KPI status text hints (warn/danger/zero), artifact pill check/cross marks, jobs filter `aria-label` and responsive class, CSS `:focus-visible` rules, skip-link CSS, `.kpi-status-hint` CSS, tablet breakpoint presence, `.jobs-filter-form`/`.jobs-filter-row` CSS, detail-grid responsive rule, mobile tap-target improvements, job detail skip-link/main, job detail action bar `role="group"`.
- **Invariants preserved**: no route changes, no backend payload changes, no queue/artifact truth drift, no action semantic changes, all existing panel tests pass, Vera context remains supplemental and read-only. This PR does not claim full WCAG compliance — it pins the specific improvements made.
- **Next safe step**: mobile runtime validation, further panel decomposition, or sorting/filtering in a follow-up PR.

## 2026-04-13 — feat(panel): polish job detail page hierarchy and evidence presentation

- **Motivation**: the job detail page displayed all sections as a flat sequence of equal-weight cards. Operators had to scroll past evidence and metadata to find action buttons, and the status was just a small badge in the header. This PR restructures the page so status, actions, and evidence are clearer and faster to navigate.
- **What shipped**: template + CSS only. Prominent status banner at top with colored left-accent border (`jd-status-banner` with `jd-status-done`/`jd-status-failed`/`jd-status-approval`/`jd-status-pending`/`jd-status-canceled`/`jd-status-running` variants), enlarged badge (`jd-badge-lg`), and integrated operator summary headline. Approval card promoted to appear immediately after operator summary for fast operator action. Actions bar (`jd-actions-section`) with retry/cancel/delete/bundle controls promoted above evidence. Evidence sections grouped: Evidence Summary → Artifact Inventory → Output → Execution Detail → Deep Evidence (plan/actions/files collapsible). Supporting context grouped lower: Failure Details, Why Stopped, Policy Rationale, Lineage, Child Summary/Jobs. Timeline, Vera Activity (supplemental), and Audit/Metadata at bottom. Evidence status badges (`badge-ok` / `badge-fail`) in evidence summary rows. Operator summary card tightened (`jd-summary-card`). Failure card (`jd-failure-card`) and stderr pre (`jd-stderr-pre`) use semantic danger styling via CSS classes instead of inline styles. Vera context card (`jd-vera-card`) uses supplemental surface styling. Mobile breakpoint for status banner and actions. No route, backend payload, queue/artifact truth, or action semantic changes.
- **Tests**: `test_job_detail_page_hierarchy_and_section_ordering` (pins status banner CSS classes, section ordering: evidence → execution → timeline → audit), `test_job_detail_approval_renders_before_evidence` (pins approval card before evidence for fast action), `test_job_detail_actions_section_renders_retry_cancel_bundle` (pins actions bar before evidence with retry/cancel/bundle controls).
- **Invariants preserved**: no route changes, no backend payload changes, no queue/artifact truth drift, no action semantic changes, all existing test assertions pass, Vera context remains supplemental and read-only.
- **Next safe step**: mobile/accessibility pass, sorting/filtering, or further panel decomposition.

## 2026-04-13 — feat(panel): improve table usability with sticky headers and clearer empty states

- **Motivation**: panel tables (approvals, active work, failed jobs, completed jobs, job browser, daemon counters) were functional but hard to scan with many rows. Empty states were inconsistent — some used plain `<p class="muted">` instead of the styled `.empty-state` container. Count columns were left-aligned. No sticky headers made scrolling large tables lose header context.
- **What shipped**: CSS + template only. Two sticky-header variants: `.table-scroll` (bounded `max-height: 28rem` for dashboard widgets) and `.table-scroll-lg` (viewport-relative `calc(100vh - 12rem)` for the standalone Job Browser). Both share `position: sticky` on `th` with per-card-tier background overrides. Right-aligned numeric cells via `.cell-count` (tabular-nums, right-align on both `th` and `td`; `font-weight: 600` scoped to `td.cell-count` only so header weight is not overridden). Enhanced `.empty-state` styling (subtle `var(--surface-3)` background for contrast in both regular and tertiary/bottom cards, dashed border, generous padding). Added `.lifecycle-cell` and `.step-progress` CSS for the jobs page lifecycle column. Applied `table-scroll` to Approval Command Center, Active Work, Failed Jobs, Completed Jobs, Mission Library, Recent Activity tables in `home.html`. Applied `table-scroll-lg` to the Job Browser table in `jobs.html`. Applied `cell-count` to Daemon Lock History count column, Panel Security Counters count column, and Mission Library steps column. Converted weak empty states: Completed Jobs `<p class="muted">` → `<div class="empty-state">No completed jobs yet.</div>`, audit section `<p class="muted">` → `<div class="empty-state">`, added Mission Library empty state `No mission templates defined.`, tightened Active Work text to "No active work." All existing empty states (`No pending queue approvals`, `No failed jobs`, `No recent lifecycle events yet`, `No jobs found for this filter`) already used `.empty-state` and are now styled with the enhanced rules. No route, backend, payload, sorting/filtering, or behavioral changes.
- **Tests**: `test_panel_table_usability.py` (14 tests) — sticky-header scroll class presence on Approvals/Completed Jobs tables (`table-scroll`) and Job Browser (`table-scroll-lg`), empty-state container rendering for active work/completed jobs/mission library/approvals/failed jobs/audit/jobs-page, count-cell alignment in Daemon Lock History/Security Counters/Mission Library, CSS class presence pins (`table-scroll`, `table-scroll-lg`, `cell-count`, `td.cell-count`, `empty-state`, `lifecycle-cell`, `step-progress`). Updated `test_panel_session_context.py` assertion for changed empty-state text ("No active work" replacing "No active jobs currently").
- **Invariants preserved**: no route changes, no backend payload changes, no queue/artifact truth drift, no semantic changes to actions or links, no sorting/filtering framework added, all existing actions still work.
- **Next safe step**: job detail page refresh, mobile/accessibility pass, or sorting/filtering in a follow-up PR.
...
- **UX tightening (smoke-test driven)**:
  - **Brain config**: `_check_brain_config` now checks *first-run readiness* — "is there at least one usable brain path?" — instead of auditing every slot individually. Unconfigured optional slots (fast, reasoning, fallback) do NOT produce individual warnings. The function always returns exactly one `brain readiness` check: ok listing usable slots, or warn with one focused fix hint. This eliminates the wall-of-warnings for typical four-slot configs where only primary is configured.
  - **Doctor noise**: `_post_setup_validation` now only surfaces doctor checks with `status == "fail"`. All warn-level doctor results (lock absent, no health events, voice unconfigured, etc.) are expected-default-state before the daemon starts and are suppressed. This eliminates `recent history: last_ok: event=- ts=-` and similar noise from the fresh-setup summary. Doctor remains the canonical surface for runtime diagnostics — the summary already recommends `voxera doctor --quick`.
- **Scope (deliberately bounded)**:
  - Add `_check_brain_config(cfg)` in `setup_wizard.py`: checks whether any configured brain slot has a resolvable API key. Returns checks in the same `{check, status, detail, hint}` dict format used by `run_quick_doctor`.
  ...
  - Add `_post_setup_validation(cfg)` in `setup_wizard.py`: orchestrates config checks + quick doctor call + summary render. Quick doctor failures are caught gracefully. Quick doctor warn checks with empty hints are filtered out — they represent expected-default-state (e.g., daemon lock absent before first start) and are noise in a fresh-setup context.
...
- **New functions in `setup_wizard.py`**:
  - `_check_brain_config(cfg: AppConfig) -> list[dict[str, str]]`: checks first-run brain readiness. Scans all configured slots, reports one `brain readiness` check: ok if any slot has a resolvable key (lists usable slots), warn with focused fix hint if none is usable. Unconfigured optional slots are silently skipped.
  ...
  - `_post_setup_validation(cfg: AppConfig) -> None`: runs `_check_brain_config` + `run_quick_doctor()` (suppressed on failure), includes only ok and fail doctor results (warn-level runtime noise is suppressed), renders combined summary.
...
- **Test coverage** — `tests/test_setup_wizard.py` (16 existing + 20 new = 36 tests): `test_check_brain_config_primary_usable`, `test_check_brain_config_no_key_ref_only_slot`, `test_check_brain_config_key_not_resolved`, `test_check_brain_config_key_found_via_keyring`, `test_check_brain_config_no_brains`, `test_check_brain_config_primary_usable_optional_unconfigured`, `test_check_brain_config_primary_usable_optional_broken_still_ok`, `test_check_brain_config_no_usable_path_shows_first_broken`, `test_check_brain_config_get_secret_exception`, `test_check_brain_config_lists_all_usable_slots`, `test_render_validation_summary_all_pass`, `test_render_validation_summary_with_warnings`, `test_render_validation_summary_with_failures`, `test_render_validation_summary_mixed_no_fake_success`, `test_post_setup_validation_suppresses_all_doctor_warns`, `test_post_setup_validation_surfaces_doctor_fail`, `test_post_setup_validation_includes_doctor_ok_in_count`, `test_post_setup_validation_handles_quick_doctor_failure`, `test_run_setup_calls_post_setup_validation`, `test_run_setup_completes_with_validation_warnings`, plus updated `test_run_setup_finish_path_ensures_services_before_launch`.
...
- **Scope (deliberately bounded)**:
  ...
  - Add `transcribe_audio_file(audio_path, flags, ..., backend=None)` and `transcribe_audio_file_async(...)` to `src/voxera/voice/input.py`: builds an `STTRequest`, selects backend via factory (or uses caller-supplied backend for model reuse), runs through canonical `transcribe_stt_request()` path. Async variant runs in a thread via `asyncio.to_thread()`.
  - Update `voice/__init__.py` exports with new public symbols.
  - Add focused pipeline/factory tests (`tests/test_voice_stt_pipeline.py`, 45 tests).
...
- **New module — `src/voxera/voice/stt_backend_factory.py`**:
  ...
  - Unrecognized backends produce a `NullSTTBackend` whose error message names the rejected identifier (e.g. `"STT backend 'google_cloud_stt' is not recognized"`), so operators can see what went wrong.
  - `STT_BACKEND_WHISPER_LOCAL = "whisper_local"` canonical identifier constant.
- **Adapter boundary change — `src/voxera/voice/stt_adapter.py`**:
  - `NullSTTBackend` now accepts an optional `reason` keyword argument (default: `"No STT backend is configured"`). The factory passes a specific reason for unrecognized backends. Existing behavior and tests are unchanged (the default covers the unconfigured case).
- **Voice input path — `src/voxera/voice/input.py`**:
  - `transcribe_audio_file(audio_path, flags, language=None, session_id=None, backend=None) -> STTResponse`: recommended entry point for audio-file transcription. Builds `STTRequest` with `input_source="audio_file"`, selects backend via `build_stt_backend(flags)` or uses caller-supplied `backend`, runs through `transcribe_stt_request()`. Always returns truthful `STTResponse`. Only `audio_file` supported — microphone and stream remain future work. Pass a pre-built `backend` to reuse an existing instance across calls and avoid re-loading heavy models like Whisper on every invocation.
  - `transcribe_audio_file_async(...)`: async variant. Runs the synchronous transcription in a thread via `asyncio.to_thread()` for use in async contexts (Vera chat, FastAPI routes). Preserves all fail-soft semantics.
  - Circular import avoided by using local imports inside `transcribe_audio_file`.
- **Test coverage** — `tests/test_voice_stt_pipeline.py` (45 tests): factory returns NullSTTBackend for no-backend/empty/whitespace/unrecognized/disabled-foundation/disabled-input; factory returns WhisperLocalBackend for whisper_local (case-insensitive, whitespace-trimmed); unrecognized backend error mentions the rejected name; unconfigured backend error is the default generic message; NullSTTBackend default reason preserved; canonical backend identifier constant; transcribe_audio_file returns unavailable for unconfigured/disabled/unknown; successful mocked transcription through full pipeline; request carries audio_path and input_source=audio_file; language and session_id pass-through; missing dependency returns unavailable; nonexistent file returns failed; empty transcript returns failed+empty_audio; backend crash is fail-soft; schema_version and request_id present; source truthfulness (only audio_file); export surface verified; config-file integration (flags loaded from JSON → factory → correct backend); env-var integration (env overrides config for backend selection); distinct request_ids per call; pre-built backend reuse (bypasses factory, same instance reused across calls, None falls through to factory); async entry point (success, unavailable when unconfigured, fail-soft on crash, returns STTResponse, accepts pre-built backend, exported from voice package).
- **Docs updated**: `docs/02_CONFIGURATION_AND_RUNTIME_SURFACES.md` (factory, pipeline entry point, async variant), `docs/09_CORE_OBJECTS_AND_SCHEMA_REFERENCE.md` (factory, transcribe_audio_file + async schema, NullSTTBackend reason parameter, backend reuse), `docs/08_TESTS_OPERATIONS_AND_CHANGE_SURFACES.md` (test listing), `docs/CODEX_MEMORY.md`.
- **Files touched**: `src/voxera/voice/stt_backend_factory.py` (new), `src/voxera/voice/stt_adapter.py` (NullSTTBackend reason parameter), `src/voxera/voice/input.py` (added transcribe_audio_file + async), `src/voxera/voice/__init__.py` (updated exports), `tests/test_voice_stt_pipeline.py` (new, 45 tests), `docs/02_CONFIGURATION_AND_RUNTIME_SURFACES.md`, `docs/09_CORE_OBJECTS_AND_SCHEMA_REFERENCE.md`, `docs/08_TESTS_OPERATIONS_AND_CHANGE_SURFACES.md`, `docs/CODEX_MEMORY.md`.
...
- **Invariants preserved**: existing voice protocol/adapter/whisper/status/foundation tests unchanged (114 tests pass); fail-soft semantics preserved; truthful behavior maintained (no fake transcripts, no fake source support); NullSTTBackend never overclaims; unrecognized backends get specific error messages; backend selection is explicit and observable; callers can reuse a backend instance to avoid per-call model reload.
- **Next safe step**: integrate `transcribe_audio_file` / `transcribe_audio_file_async` into Vera or panel flows for operator-facing audio transcription, or add microphone/stream support as a subsequent backend capability.
