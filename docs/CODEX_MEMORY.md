...
- **Tests**: 12 new tests in `tests/test_prompts.py` covering output-quality-defaults doc existence, wiring to all roles, presence in all composed prompts, automation awareness in shared prompts, unsupported features not marked active, save-vs-execute wording, plan quality guidance, non-empty structured output from all composed prompts.
