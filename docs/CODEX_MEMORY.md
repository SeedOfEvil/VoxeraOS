...
- **Architectural rule preserved**: automation remains *deferred queue submission*, not alternate execution. The `run-now` panel action processes a definition through `process_automation_definition(defn, queue_root, force=True)`, which submits via `core/inbox.add_inbox_payload(..., source_lane="automation_runner")`. The panel never bypasses the queue or executes payloads directly. Enable/disable only change the `enabled` flag; no hidden side effects.
- **New panel routes** (`src/voxera/panel/routes_automations.py`):
  - `GET /automations` — list page showing saved definitions with id, title, enabled, trigger_kind, next_run_at, last_run_at, last_job_ref. Actions: view detail, enable/disable, run-now.
  - `GET /automations/{id}` — detail page showing full definition fields, trigger config, payload template (pretty JSON), timestamps, last job ref, policy posture, created_from. Includes run history table (newest first) from existing history records. Actions: enable/disable, run-now.
...
- **Files changed**: `src/voxera/panel/routes_automations.py` (new), `src/voxera/panel/templates/automations.html` (new), `src/voxera/panel/templates/automation_detail.html` (new), `src/voxera/panel/templates/home.html` (nav link added), `src/voxera/panel/app.py` (import + registration), `tests/test_panel_automations.py` (new), `tests/test_panel_contract_snapshot.py`, `docs/01_REPOSITORY_STRUCTURE_MAP.md`, `docs/02_CONFIGURATION_AND_RUNTIME_SURFACES.md`, `docs/08_TESTS_OPERATIONS_AND_CHANGE_SURFACES.md`, `docs/ops.md`, `docs/CODEX_MEMORY.md`.
- **Tests** (`tests/test_panel_automations.py`): 31 focused tests covering: (0) panel home nav includes `/automations` link; (1) list page renders saved definitions; (2) list page empty state; (3) detail page renders definition fields; (4) enable flips to true and persists; (5) enable already-enabled redirects; (6) disable flips to false and persists; (7) disable already-disabled redirects; (8) history appears on detail page; (9) missing automation detail redirects; (10-12) missing automation enable/disable/run-now redirects; (13) list survives malformed definitions; (14) detail survives malformed history; (15) run-now submits through runner and creates inbox file; (16) run-now creates inbox file, not direct execution (pending/done/failed untouched); (17) run-now disabled automation skipped; (18-20) auth required for enable/disable/run-now; (21-22) flash messages render on list and detail pages; (23) enable preserves unrelated definition fields; (24) disable preserves unrelated definition fields; (25) run-now inbox payload carries `source_lane=automation_runner`; (26) run-now updates definition state (once_at disabled after fire); (27-29) traversal-looking and invalid automation ids handled safely; (30) detail page survives history records with non-int timestamp types.
