...
- **Tests** (`tests/test_vera_automation_lifecycle.py`): 75 focused tests covering intent classification, reference resolution (session context, explicit id, title match, pronoun, ambiguous, stale context), show/enable/disable/delete/history/run-now handlers, ambiguous fail-closed, ordinary authoring unchanged, non-automation flows unchanged, full dispatch integration scenarios, context lifecycle integration.
