...
- **What shipped**: CSS + template only. Sticky scrollable table headers via `.table-scroll` class (opt-in `max-height: 28rem`, `overflow-y: auto`, `position: sticky` on `th` with correct background per card tier). Right-aligned numeric cells via `.cell-count` class (tabular-nums, right-aligned, bold). Enhanced `.empty-state` styling (subtle `var(--surface-2)` background, dashed border, generous padding). Added `.lifecycle-cell` and `.step-progress` CSS for the jobs page lifecycle column. Applied `table-scroll` to Approval Command Center, Active Work, Failed Jobs, Completed Jobs, Mission Library, Recent Activity tables in `home.html`, and the main Job Browser table in `jobs.html`. Applied `cell-count` to Daemon Lock History count column, Panel Security Counters count column, and Mission Library steps column. Converted weak empty states: Completed Jobs `<p class="muted">` → `<div class="empty-state">No completed jobs yet.</div>`, audit section `<p class="muted">` → `<div class="empty-state">`, added Mission Library empty state `No mission templates defined.`, tightened Active Work text to "No active work." All existing empty states (`No pending queue approvals`, `No failed jobs`, `No recent lifecycle events yet`, `No jobs found for this filter`) already used `.empty-state` and are now styled with the enhanced rules. No route, backend, payload, sorting/filtering, or behavioral changes.
- **Tests**: `test_panel_table_usability.py` (14 tests) — sticky-header scroll class presence on Approvals/Completed Jobs/Job Browser tables, empty-state container rendering for active work/completed jobs/mission library/approvals/failed jobs/audit/jobs-page, count-cell alignment in Daemon Lock History/Security Counters/Mission Library, CSS class presence pins (`table-scroll`, `cell-count`, `empty-state`, `lifecycle-cell`, `step-progress`). Updated `test_panel_session_context.py` assertion for changed empty-state text ("No active work" replacing "No active jobs currently").
