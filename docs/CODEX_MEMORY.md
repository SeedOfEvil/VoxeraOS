...
- **What shipped**: template + CSS + focused tests only. No route, backend payload, queue/artifact truth, or action semantic changes.
  ...
  - **ARIA labels**: `aria-label` added to all approval/deny/cancel/retry/delete action buttons across home, jobs, and job detail templates. `aria-label="Panel sections"` on page nav. `aria-label="Job filter"` on jobs filter form. `role="group" aria-label="Queue controls"` on queue pause/resume. `role="group" aria-label="Approval actions"` / `aria-label="Job actions"` on job detail action bars. `aria-hidden="true"` on decorative nav separators.
  ...
  - **Cleanup**: removed inline `style` on jobs filter form (replaced with `.jobs-filter-form` / `.jobs-filter-row` classes). Refined `.grid-3` breakpoint cascade (no longer collapses to 1fr at 980px; transitions through 768px→540px instead).
