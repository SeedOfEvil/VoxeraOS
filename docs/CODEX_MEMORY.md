## 2026-03-23 — PR #TBD — fix(vera): fully restore answer-first checklist/planning behavior and eliminate false preview claims

- **Bug 1 — false preview claims:** LLM responses for checklist/planning turns could contain phrases like "You can see the draft in the preview pane" even though no preview exists. The guardrail was entirely skipped for answer-first turns, allowing these false claims to reach the user. After "send it", Vera truthfully reported no preview — a preview-truth contradiction.
- **Fix — soft preview-language sanitizer:** Added `_sanitize_false_preview_claims_from_answer()` in `vera_web/app.py` that strips sentences containing false preview-pane references while preserving the actual checklist/planning content. Applied instead of the heavy `_guardrail_false_preview_claim` (which replaces the entire answer) for answer-first turns.
- **Bug 2 — multi-turn planning follow-up failure:** When Vera asked for more details after a checklist request and the user provided them, the follow-up turn was reclassified as a normal turn (no planning keywords matched). The preview builder ran, failed, and the guardrail replaced the answer with "I was not able to prepare a governed preview."
- **Fix — session-persisted continuation flag:** Added `conversational_planning_active` boolean to session state (`session_store.py`). Set to True when a turn is answer-first; cleared otherwise. On the next turn, if the flag is set and the user has no save/write intent and no active preview, the turn stays in the answer-first lane.
- **Files changed:** `src/voxera/vera/session_store.py` (new session field + read/write + preserve in `append_session_turn`), `src/voxera/vera/service.py` (re-export), `src/voxera/vera_web/app.py` (sanitizer + continuation logic + flag persistence), `tests/test_vera_session_characterization.py` (6 new tests), `docs/ARCHITECTURE.md` (new section), `docs/CODEX_MEMORY.md`.
- **Tests added:** (1) preview-pane claim sanitized from checklist answer, (2) clean checklist answer passes through unmodified, (3) multi-turn clarification→details stays answer-first, (4) multi-turn then save-that creates preview, (5) checklist then send-it without save is truthful, (6) planning continuation clears when save intent detected.
- Validation: `ruff format --check .`, `ruff check .`, `mypy src/voxera`, `pytest -q` (1414 passed), `make security-check`, `make golden-check`, `make validation-check`, `make merge-readiness-check` — all pass.
- **Remaining limitations:** Continuation flag applies for one follow-up turn only (reset after any non-answer-first turn). Very long multi-turn planning chains (3+ turns of back-and-forth) will re-enter answer-first only if the user's message matches a planning keyword or the immediate prior turn was answer-first.
