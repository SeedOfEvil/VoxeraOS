...
- **Architectural rule preserved**: automation remains *deferred queue submission*, not alternate execution. The `run-now` command processes a definition through the existing `process_automation_definition` runner path, which submits via `core/inbox.add_inbox_payload(..., source_lane="automation_runner")`. The queue remains the execution boundary. `enable` / `disable` only change the `enabled` flag on the saved definition; no hidden side effects, no queue submission, no state rewriting beyond `enabled` and `updated_at_ms`.
- **New operator CLI commands** (`src/voxera/cli_automation.py`):
  ...
  - `voxera automation run-now <id>` — immediately process a single definition through the existing `process_automation_definition` runner. Submits through the canonical inbox path; the queue remains the execution boundary. Displays the runner result table.
  - `voxera automation run-due-once` — existing command, unchanged.
...
- **Files changed**: `src/voxera/cli_automation.py`, `src/voxera/automation/history.py`, `src/voxera/automation/__init__.py`, `tests/test_automation_operator_cli.py` (new), `tests/golden/voxera_help.txt`, `docs/01_REPOSITORY_STRUCTURE_MAP.md`, `docs/02_CONFIGURATION_AND_RUNTIME_SURFACES.md`, `docs/08_TESTS_OPERATIONS_AND_CHANGE_SURFACES.md`, `docs/09_CORE_OBJECTS_AND_SCHEMA_REFERENCE.md`, `docs/ops.md`, `docs/CODEX_MEMORY.md`.
- **Tests** (`tests/test_automation_operator_cli.py`): 25 focused tests covering: (1) `list` with no definitions shows placeholder; (2) `list` shows saved definitions with key fields; (3) `list` skips malformed definition files; (4) `show` renders correct definition; (5) `show` with missing id exits non-zero; (6) `enable` flips enabled to true and persists; (7) `enable` already-enabled is a no-op; (8) `enable` missing id exits non-zero; (9) `enable` preserves unrelated fields; (10) `disable` flips enabled to false and persists; (11) `disable` already-disabled is a no-op; (12) `disable` missing id exits non-zero; (13) `disable` preserves unrelated fields; (14) `history` shows linked records; (15) `history` with no records shows placeholder; (16) `history` missing id exits non-zero; (17) `history` skips malformed history files; (18) `run-now` submits through the runner; (19) `run-now` missing id exits non-zero; (20) `run-now` does not bypass queue (inbox file has `source_lane=automation_runner`); (21) `run-now` updates definition state after fire; (22–25) `list_history_records` helper: returns records for id, newest first ordering, empty when no history, rejects traversal ids.
