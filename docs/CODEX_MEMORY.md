...
- **Files changed**: `src/voxera/vera_web/app.py`, `src/voxera/vera_web/response_shaping.py`, `tests/test_vera_preview_materialization.py`, `docs/CODEX_MEMORY.md`.
- **Tests**: 9 new tests in `tests/test_vera_preview_materialization.py`: 5 positive + 16 negative parametrized unit tests for `_looks_like_previewable_automation_intent`; first-turn previewable automation no longer hard-refuses (integration); first-turn fully specified direct draft path is unchanged (integration); regression tests verifying weather, writing, and simple file ops are not hijacked.
