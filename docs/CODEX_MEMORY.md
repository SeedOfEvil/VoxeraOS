...
- **Import guidance**: pure predicates from `execution_mode.py` and `conversational_checklist.py` should be imported from those modules directly, not from `app.py`. Only the binding wrappers that curry dependencies live in `app.py`.
- **Files changed**: `src/voxera/vera_web/app.py`, `tests/test_vera_web.py`, `tests/test_file_intent.py`, `tests/test_vera_session_characterization.py`, `docs/CODEX_MEMORY.md`, `docs/HOTSPOT_AUDIT_EXTRACTION_ROADMAP.md`.
