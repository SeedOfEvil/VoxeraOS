...
from voxera.cli_common import CONFIG_GUARD_MESSAGE
...
def test_vera_guarded_when_config_missing(monkeypatch, tmp_path):
    ...
    assert CONFIG_GUARD_MESSAGE in _strip_ansi(result.stdout)
...
def test_panel_proceeds_when_config_present(monkeypatch, tmp_path):
    _with_present_config(monkeypatch, tmp_path)
    monkeypatch.setattr("voxera.cli.panel_impl", lambda **kw: None)
    result = runner.invoke(cli.app, ["panel"])
    assert result.exit_code == 0
...
def test_setup_unguarded(monkeypatch, tmp_path):
    ...
    assert result.exit_code == 0
    assert CONFIG_GUARD_MESSAGE not in _strip_ansi(result.stdout)
