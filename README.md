...
- Panel operator assistant (`/assistant`) adds an advisory-only “Ask Voxera” surface grounded in live queue/health/approvals/failure context; questions are enqueued as dedicated `assistant_question` jobs and answered through Voxera Queue (read-only advisory lane, no tool execution/approval/mutation) using configured brain/provider with deterministic primary→fallback attempts. Assistant response artifacts now persist advisory metadata (`provider`, `model`, `fallback_used`, `fallback_reason`, `fallback_from`, `advisory_mode`, `degraded_reason`) so operators can see exactly how answers were produced. Assistant now supports compact multi-turn thread continuity via persisted per-thread turns in queue artifacts.
