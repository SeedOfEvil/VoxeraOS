...
- Panel hygiene page (`/hygiene`) shows the latest `voxera queue prune --dry-run --json` and `voxera queue reconcile --json` snapshots and provides operator-trigger buttons for both actions with in-page async refresh.
