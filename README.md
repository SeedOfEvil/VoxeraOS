...
- Singular save-by-reference phrasing like `save that`, `save it`, `put that in a note`, or `create a note from that` now resolves against a bounded recent-saveable-assistant-artifact layer rather than lane-specific exceptions.
- The saveable-assistant-artifact layer is deterministic and session-bounded: meaningful assistant-displayed informational content (for example weather answers, concise factual replies, explanations, summaries, articles, and code explanations) is eligible, while courtesy/boilerplate/control replies are excluded.
...
- Transform-style follow-ups over investigation-derived text (for example `write a short article based on that summary`, `turn that summary into a concise article`, or `expand that summary into a writeup`) now route into the governed writing lane instead of reusing the existing derived save artifact as-is.
...
- This save-by-reference resolution is intentionally bounded to recent assistant content in the current session only; it is not cross-session memory, and ambiguous/no-content references fail closed with a clear message.
...
- Prompts like `What's the weather like?`, `How's the weather today?`, `What's the weather in Calgary?`, and `Current weather in Calgary AB` now use a lightweight live lookup path backed by Brave Search API plus bounded weather-result extraction from trusted surfaced sources.
...
- Current/live weather facts must come from the Brave-backed live lookup path. If the lookup fails, Brave is not configured, or the place cannot be resolved strongly enough, Vera says so explicitly and does not invent temperatures, highs/lows, or conditions.
...
- For investigation-derived summaries specifically, plain `save it` / `save it as ...` still saves the current derived artifact directly, while transform requests create a new authoritative prose preview first and bind later save/save-as follow-ups to that newer writing artifact.
