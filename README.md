...
- Prompts like `What's the weather like?`, `How's the weather today?`, `What's the weather in Calgary?`, and `Current weather in Calgary AB` now use a lightweight structured live lookup path backed by Open-Meteo weather data.
...
- Current/live weather facts must come from the structured live lookup path. If the lookup fails or the place cannot be resolved strongly enough, Vera says so explicitly and does not invent temperatures, highs/lows, or conditions.
