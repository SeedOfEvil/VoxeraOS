...
- ✅ Queue daemon for mission/goal JSON jobs plus approval inbox (`pending/approvals/*.approval.json`)
  - Daemon orchestration remains in `src/voxera/core/queue_daemon.py`; persisted job-state sidecar helpers are in `src/voxera/core/queue_state.py`; bucket-transition helpers are in `src/voxera/core/queue_paths.py`.
...
- Panel operator assistant (`/assistant`) adds an advisory-only “Ask Voxera” surface grounded in live queue/health/approvals/failure context; questions are enqueued as dedicated `assistant_question` jobs and answered through Voxera Queue (read-only advisory lane, no tool execution/approval/mutation) using configured brain/provider with deterministic primary→fallback advisory attempts (fallback only on explicit retryable classes: timeout/auth/rate-limit/malformed/network). Assistant artifacts now persist compact response metadata (`provider`, `model`, `fallback_used`, `fallback_reason`, `advisory_mode`, `degraded_reason` when relevant), and `/assistant` surfaces this metadata so operators can see queue-backed vs degraded advisory mode at a glance. Assistant now supports compact multi-turn thread continuity via persisted per-thread turns in queue artifacts.
