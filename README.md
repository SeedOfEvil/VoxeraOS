...
- Panel operator assistant (`/assistant`) adds an advisory-only “Ask Voxera” surface grounded in live queue/health/approvals/failure context; responses use a bounded first-person control-plane voice ("from inside Voxera") to explain current interpretation and suggest next operator actions without executing from chat.
